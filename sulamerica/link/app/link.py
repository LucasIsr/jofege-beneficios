...
class Link:
    ...
    def __init__(self, url: str, sleep: int, driver: Literal['Chrome', 'Firefox'] = 'Chrome', headless: bool = False, prod: bool = True) -> None:
        ...
        self.prod = prod
        self.user_data_dir = None  # Inicialize o atributo aqui
    ...
    def open_link(self) -> None:
        ...
        try:
            if self.driver == "Chrome":
                ...
                if self.prod:
                    self.driver = uc.Chrome(options=options, service=ChromeService('/app/driver/chromedriver'))
                else:
                    self.driver = uc.Chrome(options=options, service=ChromeService())
    ...
    def click_image(self, image_path: str) -> None:
        """
        Método usando o opencv para identificação de imagem
        """
        try:
            threshold=0.9
            screenshot_path = f'{os.getcwd()}/shot_1.png'
            ...
            if max_val >= threshold:
                ...
                raise ExecutionException('Imagem não encontrada')
        except Exception as e:
            raise ExecutionException(str(e))
    ...
    def find_elements(self, xpath: str):
        """
        Método utilizado para retornar uma lista de elementos baseados no xpath
        """
        try:
            self._delay()
    ...
    def scroll_to(self, target: str | int = 'bottom') -> None:
        """
        MÉTODO UTILIZADO PARA REALIZAR SCROLL NA PÁGINA.
        - Se for passado 'bottom', rola até o fim da página.
        - Se for passado um número inteiro, rola até essa posição em pixels (eixo Y).
        - Se for passado um xpath (string que comece com // ou (//), rola até o elemento.
        """
        try:
            self._delay()
            
            if isinstance(target, int):
                # SCROLL PARA UMA ALTURA ESPECÍFICA
                self.driver.execute_script(f"window.scrollTo(0, {target});")

            elif isinstance(target, str) and (target.startswith("//") or target.startswith("(")):
                ...
                raise ExecutionException("Parâmetro 'target' inválido para scroll.")
